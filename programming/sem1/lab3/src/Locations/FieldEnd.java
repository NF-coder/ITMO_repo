...
public class FieldEnd extends Location{
    ...
    public Location run(){
        ...
        if (character.getEnergy() < this.totalLenght){
            System.out.println("В какой-то момент он окончательно выбился из сил, сел посреди поля и стал ожидать чего-то, сам не зная чего.");
        } else if (character.getEnergy() == this.totalLenght) {
            System.out.println("Он еле-еле смог доползти до конца поля и ступив на твёрдую землю он сразу лёг в траву и уснул");
        } else {
            System.out.println("Как и следовало ожидать, ему все же удалось в конце концов добраться до края картофельного поля.\n" +
            ...
            if (rnd.nextFloat(0,1) < 0.7f){
                ...
                System.out.println("и в тот же момент ощутил доносившийся откуда-то запах " +
                        SmellType.values()[pickSmell] +
                        ". От этого запаха на него словно повеяло теплом и домашним уютом. \n");
            } else {
                System.out.println(". Он ещё никогда не был так рад ощущать траву под ногами.");
            }
        }
